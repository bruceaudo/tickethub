...
import Profile from '../public/profile-pic.avif'
    ...
    <div className='flex items-center gap-2 sm:gap-5'>
      ...
      isOpen: true
        ...
        <div className="flex lg:hidden">
          {!session?.user ? <div><button className='shadow-none bg-transparent' onClick={checkIfLoggedIn} className='mr-5'>Sell</button>{providers && Object.values(providers).map(provider=> <button type='button' key={provider.name} onClick={()=>signIn(provider.id)} className="rounded-lg px-3 w-fit py-2 bg-[#0077be] text-white text-sm">
                Sign up / Sign in
          </button>)}</div> : <div className='flex items-center'><Link className='mr-5' href={'/sell/dashboard'}>Sell</Link><span onClick={() => setIsOpen(prev => !prev)} className="flex gap-[2px] relative items-center cursor-pointer">
